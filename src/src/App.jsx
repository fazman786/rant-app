    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .vr-idle{display
        ...
        export default function App(){
          ...
          useEffect(()=>{
            if(!authUser?.id) return;
            fetchFollowing(authUser.id).then(ids=>setFollowingIds(ids));
          },[authUser?.id]);
          ...
          const confirmBoost=useCallback(async()=>{
            if(!profile)return;
            if(!profile.is_pass){const spent=spendCoins(profile.username,COIN_COST_BOOST);if(!spent){setBoostTarget(null);return;}}
            await boostRant(boostTarget.id);
            setRants(prev=>prev.map(r=>r.id===boostTarget.id?{...r,boosted:true}:r));
            await pushNotification(authUser.id,"⚡","Rant boosted! Pinned for 1 hour.");
            setBoostTarget(null);
                ...
                <header className="header">
                  <div className="header-top">
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      {(profile?.streak||0)>1&&<div className="header-streak">🔥 {profile.streak}</div>}
                      <CoinBadge username={profile?.username||""}/>
                      <button className="search-icon-btn" onClick={()=>setShowSearch(s=>!s)}>{showSearch?"✕":"🔍"}</button>
                    </div>
                  </div>
                  {tab==="feed"&&!showSearch&&(
                    <div className="filter-bar">

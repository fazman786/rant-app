...
function PassModal({onClose,onActivate}){
  ...
  const PRICE_IDS={monthly:"pri_01ksj77rntt2xtz632tzfmkz9k",yearly:"pri_01ksj78mpkhbptwnewnhhqqmar"};
  const go=async()=>{
    setProcessing(true);
    try{
      const res=await fetch("https://sandbox-api.paddle.com/transactions",{
        method:"POST",
        headers:{"Content-Type":"application/json","Authorization":`Bearer ${import.meta.env.VITE_PADDLE_KEY}`},
        body:JSON.stringify({
          items:[{price_id:PRICE_IDS[plan],quantity:1}],
          checkout:{url:"https://rantit.app/?activated=true"}
        })
      });
      const data=await res.json();
      const checkoutUrl=data?.data?.checkout?.url;
      if(!checkoutUrl)throw new Error(data?.error?.detail||"Failed to create checkout");

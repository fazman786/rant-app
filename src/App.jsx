...
function PassModal({onClose,onActivate}){
  ...
  const VARIANT_IDS={monthly:"1707701",yearly:"1707698"};
  const go=async()=>{
    setProcessing(true);
    try{
      const res=await fetch("https://api.lemonsqueezy.com/v1/checkouts",{
        ...
        headers:{
          ...
          "Authorization":`Bearer ${import.meta.env.VITE_LEMONSQUEEZY_KEY}`
        },
        body:JSON.stringify({
          data:{
            type:"checkouts",
            attributes:{
              checkout_options:{
                dark:true,
                success_url:"https://rantit.app/?activated=true"
              }
            },
            relationships:{
              store:{data:{type:"stores",id:"388158"}},
      ...
      if(!url)throw new Error(JSON.stringify(data?.errors||"Failed to create checkout"));

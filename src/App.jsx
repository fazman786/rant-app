...
async function getAIValidation(rantText, category) {
  ...
  try {
    const res = await fetch("https://tgzfokaztzwracldnhae.supabase.co/functions/v1/claude-proxy",{
      method:"POST",
...
async function pushNotification(userId, icon, message) {
  ...
  // Send OneSignal push notification using external_id (Supabase user ID)
  try {
...
function ShareCard({rant,onClose}){
  ...
  const playStoreUrl="https://play.google.com/store/apps/details?id=app.rantit.app";
  const txt=`${rant.type==="voice"?"🎙 Voice rant":rant.text}\n\n🔥 ${rant.heat} heat · ${totalR} reactions\n\n📱 Download RANT on Google Play:\n${playStoreUrl}\n\nScream into the void 👉 rantit.app`;
  const copy=()=>navigator.clipboard?.writeText(txt).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),2000);});
  const share=()=>navigator.share?navigator.share({title:"RANT",text:txt,url:playStoreUrl}):copy();
  return(
        ...
        <div className="share-preview">
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              <span style={{fontSize:11,color:"var(--accent)",fontFamily:"var(--display)",letterSpacing:1}}>🔥 {rant.heat} HEAT</span>
            </div>
          </div>
          <div className="sc-cat">{cat?.emoji} {cat?.label}</div>
          <p className="sc-text">{rant.type==="voice"?"🎙 Voice rant":`"${rant.text?.slice(0,120)}${rant.text?.length>120?"…":""}"`}</p>
          <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:12}}>
            {REACTIONS.map(r=>{const c=rant.reactionCounts?.[r.id]||0;return c>0?<span key={r.id} style={{fontSize:13,color:"var(--muted)",display:"flex",alignItems:"center",gap:3}}>{r.emoji}<span style={{fontSize:11}}>{c}</span></span>:null;})}
          </div>
          <div style={{borderTop:"1px solid var(--bd)",paddingTop:10,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{fontFamily:"var(--display)",fontSize:13,color:"var(--accent)",letterSpacing:2}}>rantit.app</div>
              <div style={{fontSize:10,color:"var(--dim)",letterSpacing:1,marginTop:2}}>scream into the void</div>
            </div>
            <a href={playStoreUrl} target="_blank" rel="noreferrer" style={{background:"var(--s2)",border:"1px solid var(--bd)",borderRadius:8,padding:"6px 10px",fontSize:10,color:"var(--muted)",textDecoration:"none",fontFamily:"var(--display)",letterSpacing:1}}>▶ GET THE APP</a>
          </div>
        </div>
        <div style={{display:"flex",gap:10}}>
          <button className="share-action-btn" onClick={copy}>{copied?"✓ COPIED":"📋 COPY"}</button>
          <button className="share-action-btn" onClick={share}>📤 SHARE</button>
        </div>
      </div>
    </div>
  );
...
function AuthScreen({onAuth}){
  const ADJECTIVES=["Angry","Ranting","Chaotic","Void","Savage","Bitter","Wild","Stormy","Rogue","Spicy","Gloomy","Blazing","Toxic","Hollow","Restless"];
  const NOUNS=["Toast","Cactus","Noodle","Goblin","Phantom","Vortex","Waffle","Rebel","Ghost","Pickle","Crater","Wraith","Comet","Drifter","Tempest"];
  const generateUsername=()=>{const adj=ADJECTIVES[Math.floor(Math.random()*ADJECTIVES.length)];const noun=NOUNS[Math.floor(Math.random()*NOUNS.length)];const num=Math.floor(Math.random()*99)+1;setUsername(`${adj}${noun}${num}`);};
    ...
    <div className="auth-screen">
      ...
      <div className="auth-card">
        ...
        <div className={`auth-form${shake?" shake":""}`}>
          <div className="auth-field"><label className="auth-lbl">USERNAME</label><div style={{display:"flex",gap:8,alignItems:"center"}}><input className="auth-inp" style={{flex:1}} placeholder="yourname" value={username} onChange={e=>setUsername(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submit()} autoCapitalize="none" autoCorrect="off"/>{mode==="signup"&&<button type="button" onClick={generateUsername} style={{background:"var(--s2)",border:"1px solid var(--bd)",borderRadius:8,padding:"0 12px",height:44,fontSize:18,cursor:"pointer",flexShrink:0}} title="Generate random username">🎲</button>}</div>{mode==="signup"&&<p style={{fontSize:11,color:"var(--dim)",marginTop:4,letterSpacing:1}}>tap 🎲 for a random anonymous name</p>}</div>
...
export default function App(){
  ...
  const handlePost=useCallback(async(data)=>{
    ...
    if(replyingTo){
      ...
      if(replyingTo.author_id&&replyingTo.author_id!==authUser.id){
        await pushNotification(replyingTo.author_id,"💬",`@${profile.username} replied to your rant!`);
      }
    } else {
  ...
  const handleReact=useCallback(async(rant,reactionType="heat")=>{
    ...
    if(!wasReacted&&rant.author_id!==authUser.id){
      ...
      await pushNotification(rant.author_id,reaction?.emoji||"🔥",`@${profile.username} reacted ${reaction?.emoji||"🔥"} to your rant!`);

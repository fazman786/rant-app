...
const OneSignalManager = {
  ...
  async sendNotification(userId, title, message) {
    // Store player ID in Supabase for server-side sending
    // Notifications are triggered via Supabase and OneSignal REST API
    try {
...
function ComposeSheet({onClose,onPost,profile,onOpenPass,replyingTo,prefillText}){
  ...
  const charPercent = Math.min(100,(text.length/MAX_CHARS)*100);
          ...
          <div className="compose-voice-area">
            ...
            {!profile?.is_pass&&(
              <button className="compose-pass-strip" onClick={()=>{onClose();onOpenPass();}}>
                ...
                <span>Unlock 5-min voice with Rant Pass</span>
                <span style={{color:"var(--gold)"}}>›</span>
              </button>
            )}
          </div>
...
function PassModal({onClose,onActivate}){
  ...
  const go=async()=>{
    ...
    try{
      ...
      localStorage.setItem('pass_pending','1');
...
// ─── PROFILE ─────────────────────────────────────────────────────────────────
// ─── USER PROFILE SCREEN ──────────────────────────────────────────────────────
function UserProfileScreen({username, currentUserId, followingIds, onFollow, onUnfollow, onClose, cardProps}){
  ...
  return(
    <div className="user-profile-screen">
      <div className="up-header">
        <button className="up-back" onClick={onClose}>← BACK</button>
        <span className="up-title">@{username}</span>
        <div style={{width:60}}/>
      </div>
      {loading?(
        <div style={{textAlign:"center",padding:"40px 0",color:"var(--muted)",fontFamily:"var(--display)",letterSpacing:2}}>LOADING…</div>
      ):userProfile?(
        <div className="up-content">
          <div className="up-profile-card">
            <div className="up-avatar">{username.slice(0,2).toUpperCase()}</div>
            <div className="up-info">
              <div className="up-username">@{username}</div>
              {userProfile.is_pass&&<span className="bdg-pass-lg">✦ RANT PASS</span>}
          ...
          {userRants.length===0?(
            <div className="empty-state"><span style={{fontSize:40}}>😶</span><p className="empty-txt">NO RANTS YET</p></div>
          ):(
            <div style={{display:"flex",flexDirection:"column",gap:10,padding:"0 16px 100px"}}>
              {userRants.map(r=><RantCard key={r.id} rant={r} {...cardProps} depth={0}/>)}
            </div>
          )}
        </div>
      ):(
        <div className="empty-state"><span style={{fontSize:40}}>👁</span><p className="empty-txt">USER NOT FOUND</p></div>
      )}
    </div>
  );
}
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    const ch=sb.channel("rants-live")
      ...
      .on("postgres_changes",{event:"UPDATE",schema:"public",table:"rants"},payload=>{
        setRants(prev=>prev.map(r=>{if(r.id===payload.new.id){const updated=normalizeRant({...r,...payload.new},authUser?.id);return{...updated,myReactions:r.myReactions,reactionCounts:r.reactionCounts};}return r;}));
      })
      .subscribe();
  ...
  if(viewingUser)return<><CSS/><UserProfileScreen username={viewingUser} currentUserId={authUser?.id} followingIds={followingIds} onFollow={handleFollow} onUnfollow={handleUnfollow} onClose={()=>setViewingUser(null)} cardProps={cardProps}/></>;
